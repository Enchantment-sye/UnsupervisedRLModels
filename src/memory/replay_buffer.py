...
class PathBufferEx:
    ...
    def __len__(self):
        return self._transitions_stored + 1
        ...
        for key, array in path.items():
            if self._pixel_dim is not None and key in self._pixel_keys:
                ...
                if pixel_key not in self._buffer:
                    self._buffer[pixel_key] = np.empty((self._capacity, self._pixel_dim), dtype=np.uint8)
    ...
    def sample_transitions(self, batch_size):
        ...
        n = int(self._transitions_stored)
        if n <= 0:
            raise RuntimeError("Replay buffer is empty; cannot sample transitions.")

        # RL 里采样通常允许重复（replace=True），行为与原来一致
    ...
    def sample_paths(self, batch_size, replace=False):
        """Sample stored paths without mutating buffer state."""
        n_paths = len(self._path_segments)
        if n_paths <= 0:
            raise RuntimeError("Replay buffer has no stored paths; cannot sample paths.")

        if replace:
            sample_size = int(batch_size)
            idxs = np.random.choice(n_paths, sample_size, replace=True)
        else:
            sample_size = min(int(batch_size), n_paths)
            idxs = np.random.choice(n_paths, sample_size, replace=False)

        segments = list(self._path_segments)
        return [self._reconstruct_path(segments[idx]) for idx in idxs]
    ...
    def _reconstruct_path(self, path_segments):
        first_seg, second_seg = path_segments
        if self._pixel_dim is not None:
            path = {}
            keys = set(self._buffer.keys())
            for key in self._pixel_keys:
                ...
                keys.remove(pixel_key)
                keys.remove(state_key)
                pixel_values = self._slice_segments(self._buffer[pixel_key], first_seg, second_seg)
                state_values = self._slice_segments(self._buffer[state_key], first_seg, second_seg)
                if state_values.shape[1] != 0:
                    path[key] = np.concatenate([pixel_values, state_values], axis=1)
                else:
                    path[key] = pixel_values
            for key in keys:
                path[key] = self._slice_segments(self._buffer[key], first_seg, second_seg)
            return path
        return {
            key: self._slice_segments(buf_arr, first_seg, second_seg)
            for key, buf_arr in self._buffer.items()
        }

    @staticmethod
    def _slice_segments(array, first_seg, second_seg):
        first_part = array[first_seg.start:first_seg.stop]
        if second_seg.stop == 0:
            return first_part.copy()
        second_part = array[second_seg.start:second_seg.stop]
        return np.concatenate([first_part, second_part], axis=0).copy()
