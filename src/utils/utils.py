...
def log_performance_ex(itr, batch, discount, additional_records=None, additional_prefix=''):
    ...
    for trajectory in batch.split():
        ...
        if 'success_tasks' in trajectory.env_infos:
            success_tasks.append(float(trajectory.env_infos['success_tasks'].any()))
    ...
    return {
        ...
        'success_rate': None if not success else np.mean(success),
        'success_tasks': None if not success else np.mean(success_tasks),
    }
...
def get_torch_concat_obs(obs, skill, dim=1):
    ...
    if not torch.is_tensor(skill):
        skill = torch.as_tensor(skill, device=obs.device, dtype=obs.dtype)
    else:
        skill = skill.to(device=obs.device, dtype=obs.dtype)
    ...
    # 展平 trailing dims： (B, H, W, C) -> (B, H*W*C)
    if obs.dim() > 2:
        obs = obs.reshape(obs.shape[0], -1)
    if skill.dim() > 2:
        skill = skill.reshape(skill.shape[0], -1)

    return torch.cat([obs, skill], dim=dim)


def get_np_concat_obs(obs, skill):
    """
    Patched:
      - rollout 常见是单步 1D 或 HWC 图像；这里做更鲁棒的拼接
      - 若都是 2D batch，则沿 feature 维拼（axis=1）
      - 否则 fallback：全部 flatten 后拼接（axis=0）
    """
    obs_arr = np.asarray(obs)
    skill_arr = np.asarray(skill)

    if obs_arr.ndim == 2 and skill_arr.ndim == 2:
        return np.concatenate([obs_arr, skill_arr], axis=1)

    if obs_arr.ndim == 1 and skill_arr.ndim == 1:
        return np.concatenate([obs_arr, skill_arr], axis=0)

    # fallback：例如 obs 是 HWC / CHW
    return np.concatenate([obs_arr.reshape(-1), skill_arr.reshape(-1)], axis=0)
...
def infer_video_grid_cols(num_videos, n_cols=None):
    if n_cols is not None:
        return max(1, int(n_cols))
...
def prepare_video(v, n_cols=None):
    ...
    n_cols = infer_video_grid_cols(v.shape[0], n_cols=n_cols)
    if v.shape[0] % n_cols != 0:
        len_addition = n_cols - v.shape[0] % n_cols
        if v.shape[0] > 0:
            pad_video = np.repeat(v[-1:], len_addition, axis=0)
        else:
            pad_video = np.zeros(shape=(len_addition, t, c, h, w), dtype=v.dtype)
        v = np.concatenate((v, pad_video), axis=0)
...
def save_video(snapshot_dir, step_itr, label, tensor, fps=15, n_cols=None):
    if mpy is None:
        raise ImportError(
            "moviepy is required for save_video()/record_video(). "
            "Install moviepy or disable video recording."
        )
    def _to_uint8(t):
...
def trajectories_to_video_tensor(trajectories, skip_frames=1, shape=(64, 64)):
    def _sanitize_render_sequence(render):
        if render.shape[0] == 0:
            return None

        valid_mask = np.any(render.reshape(render.shape[0], -1) != 0, axis=1)
        if not np.any(valid_mask):
            return None

        render = np.asarray(render).copy()
        first_valid_idx = int(np.argmax(valid_mask))
        first_valid_frame = render[first_valid_idx].copy()
        if first_valid_idx > 0:
            render[:first_valid_idx] = first_valid_frame

        last_valid_frame = first_valid_frame
        for idx in range(first_valid_idx, render.shape[0]):
            if valid_mask[idx]:
                last_valid_frame = render[idx].copy()
            else:
                render[idx] = last_valid_frame
        return render
    ...
    for trajectory in trajectories:
        ...
        if render.ndim >= 5:
            ...
            if len(render) == 0:
                render = np.zeros((0, shape[0], shape[1], 3), dtype=np.uint8)
            else:
                render = np.concatenate(render, axis=0)
        render = _sanitize_render_sequence(render)
        if render is None:
            continue
        renders.append(render)

    if not renders:
        return np.zeros((0, 0, 3, shape[0], shape[1]), dtype=np.uint8)

    max_length = max([len(render) for render in renders])
    for i, render in enumerate(renders):
        pad_count = max_length - render.shape[0]
        if pad_count > 0:
            if render.shape[0] > 0:
                pad_frame = np.repeat(render[-1:], pad_count, axis=0)
            else:
                pad_frame = np.zeros((pad_count, *render.shape[1:]), dtype=render.dtype)
            render = np.concatenate([render, pad_frame], axis=0)
        renders[i] = render
        renders[i] = renders[i][::skip_frames]
    renders = np.array(renders)
    if renders.size == 0:
        return np.zeros((len(renders), 0, 3, shape[0], shape[1]), dtype=np.uint8)
    if renders.ndim == 3:
        flat_dim = renders.shape[-1]
        pixels_per_frame = shape[0] * shape[1]
        if flat_dim % pixels_per_frame != 0:
            raise ValueError(
                f"Video shape mismatch: flat observation dim {flat_dim} cannot be reshaped "
                f"to frames of shape {shape}. Check render_size / record_video shape."
            )
    renders = renders.reshape((renders.shape[0], renders.shape[1], *shape, -1)).transpose((0, 1, 4, 2, 3))[:, :, -3:, :, :]
    return renders
