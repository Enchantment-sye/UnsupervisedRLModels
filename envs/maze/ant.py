...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        return (
            ...
            {
                'xy': self.get_xy(),
                'coordinates': xy_before.astype(np.float32),
                'next_coordinates': self.get_xy().astype(np.float32),
                'prev_qpos': prev_qpos,
                'prev_qvel': prev_qvel,
                'qpos': qpos,
                'qvel': qvel,
    ...
    def calc_eval_metrics(self, trajectories, is_option_trajectories=False):
        return compute_locomotion_coverage_metrics(trajectories, coord_dims=2)
