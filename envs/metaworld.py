...
def _get_metaworld_goal_observable_registry():
    """Return the installed MetaWorld goal-observable registry and task suffix."""
    try:
        from metaworld.envs import ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE

        return ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE, "-v2-goal-observable"
    except ImportError:
        import metaworld

        registry = getattr(metaworld, "ALL_V3_ENVIRONMENTS_GOAL_OBSERVABLE")
        return registry, "-v3-goal-observable"


def _make_metaworld_env(name, seed=None):
    registry, suffix = _get_metaworld_goal_observable_registry()
    task = f"{name}{suffix}"
    env_cls = registry[task]
    try:
...
class MetaWorld:
    ...
    def __init__(self, name, seed=None, action_repeat=1, size=(64, 64), camera='corner', use_gripper=False, flatten_obs=False):
        ...
        self._env = _make_metaworld_env(name, seed=seed)
    ...
    def step(self, action):
        ...
        info.update({
            ...
            "ori_obs": prev_state,        # for METRA/IOD compatibility
            "next_ori_obs": next_state,   # for METRA/IOD compatibility
        })
        image = self.render()
        obs = {
            "reward": reward,
            ...
            #     *self._size, mode="offscreen"zz, camera_name=self._camera
            # ),
    ...
    def reset(self):
        ...
        self._prev_state, info = self._env.reset()
        image = self.render()
        obs = {
            "reward": 0.0,
            "is_first": True,
    ...
    def render(self, mode='offscreen'):
        ...
        obs = cv2.resize(self._env.render(), self._size)#  camera_name="corner2", resolution=(640, 480)
        if self.flatten_obs:
            obs = obs.flatten()
        return obs
...
class MultiViewMetaWorld:
    ...
    def render(self, mode='offscreen'):
        ...
        for camera in self.camera_keys:
            ...
            imgs.append(cv2.resize(self._env.render(), self._size))
...
class ViewMetaWorld:
    ...
    def __init__(self, name, seed=None, action_repeat=1, size=(64, 64), camera=None, viewpoint_mode='shake', viewpoint_randomization_type='strong', use_gripper=False):
        ...
        self._env = MetaWorldMovableCameraWrapper(
            _make_metaworld_env(name, seed=seed),
            seed,
            size[0],
            viewpoint_mode=viewpoint_mode,
            viewpoint_randomization_type=viewpoint_randomization_type,
        )
        self._size = size
        self._action_repeat = action_repeat
        self._use_gripper = use_gripper
