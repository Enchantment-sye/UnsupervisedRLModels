...
try:
    import gym
except ImportError:  # GalaxeaManipSim/metra_galaxea ships Gymnasium only.
...
class NormalizeAction:
    ...
    def safety_denormalize_action(self, action):
        value = action.get(self._key, action) if isinstance(action, dict) else action
        value = np.asarray(value, dtype=np.float32)
        orig = (value + 1) / 2 * (self._high - self._low) + self._low
        orig = np.where(self._mask, orig, value)
        return {self._key: orig.astype(np.float32)} if isinstance(action, dict) else orig.astype(np.float32)

    def safety_normalize_action(self, action):
        value = action.get(self._key, action) if isinstance(action, dict) else action
        value = np.asarray(value, dtype=np.float32)
        denom = np.where(np.abs(self._high - self._low) > 1e-8, self._high - self._low, 1.0)
        norm = (value - self._low) / denom * 2.0 - 1.0
        norm = np.where(self._mask, norm, value)
        return {self._key: norm.astype(np.float32)} if isinstance(action, dict) else norm.astype(np.float32)

    def safety_physical_action_bounds(self):
        return self._low.astype(np.float32).copy(), self._high.astype(np.float32).copy()
