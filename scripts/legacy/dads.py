...
from metra import DRQ_METRAAgent


class DADS(DRQ_METRAAgent):
    def __init__(
            self,
            **kwargs,
    ):
        super().__init__(**kwargs)
        self._sd_batch_norm = kwargs['sd_batch_norm']
        self._skill_dynamics_obs_dim = kwargs['skill_dynamics_obs_dim']
    ...
    def _train_components(self, epoch_data):
        ...
        for _ in range(self._trans_optimization_epochs):
            ...
            self._optimize_op(tensors, v, self.total_train_steps)
    ...
    def _update_loss_sd(self, tensors, v):
        ...
        sd_input = self._get_concat_obs(self._process_sd_input(v['obs']), v['skills'])
    ...
    def _update_rewards(self, tensors, v):
        with torch.no_grad():
            ...
            alt_options_shape = (obs_repeated.size(0), self.dim_skill)
            if self.discrete:
                alt_options = torch.randint(self.dim_skill, (obs_repeated.size(0),)).to(self.device)
                alt_options = torch.eye(self.dim_skill, device=self.device)[alt_options]
            else:
